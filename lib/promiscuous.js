...
var config,
    _ = function () {}, //function placeholder
    defaultConfig = {
        promise: null,
        callbackStyles: {
...
//Turn a value into a fulfilled promise if passed value is not already a promise
function promisify (val) {
    if (typeof val.then === 'function') {
        return val;
    } else {
        return promiseFactory(function (fulfill) {
            fulfill(val);
        });
    }
}
...
//Apply a function to the then of a collection of promises.
//Arguments ordered for composition and partial application.
function map (func, promiseCollection) {
    return all(promiseCollection.map(function (promise) {
        return promisify(promise).then(func);
    }));
}
...
function all (promiseCollection) {
    ...
    promiseCollection.forEach(function (p, i) {
        promisify(p).then(fulfiller(i), reject);
    });
...
function sequence () {
    var args = toArray(arguments),
        ...
        values = tasks.reduce(function (accumulator, task, i) {
                ...
                return accumulator;
            }, [promisify(firstTask.apply(firstTask, args))]);
...
//Delay the execution of a promise function
function delay (time, func) {
    return promiseFactory(function (fulfill, reject) {
        var timer = setTimeout(function () {
            try {
                fulfill(func.call(null));
            } catch (e) {
                reject(e.message);
            }
        }, time);
...
//Attempt to get a success response from a promise returning function x number of times.
function attempt (tries, func) {
    var tryIt = function (fulfill, reject) {
        func.apply(null).then(fulfill, function () {
            if (tries < 1) {
                reject('Max attempts reached.');
            } else {
                --tries;
                tryIt(fulfill, reject);
            }
        });
    };
    return promiseFactory(function (fulfill, reject) {
...
function forever () {
    var args = toArray(arguments),
        ...
        recurse = function () {
            try {
                return promisify(func.apply(null, args)).then(recurse, rejector);
            } catch (e) {
                rejector(e);
            }
        };
...
function foreverPipe () {
    var args = toArray(arguments),
        ...
        recurse = function () {
            try {
                return promisify(func.apply(null, arguments)).then(recurse, rejector);
            } catch (e) {
                rejector(e);
            }
        };
